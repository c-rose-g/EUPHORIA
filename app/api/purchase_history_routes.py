...
def add_purchase_history(user_id):
    shopping_cart = ShoppingCart.query.filter_by(user_id=user_id).first().to_dict()
# what we need to do add evertthing in shopping cart into purchase history, and empty out the cart simple? lets hope
    shopping_cart_items = ShoppingCartItem.query.filter_by(shopping_cart_id = shopping_cart['id'])
    ...
    for items in shopping_cart_items:
        if items:
            ...
            return user_purchase_history.to_dict()
